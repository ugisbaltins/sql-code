...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      ...
      WHERE release_date > 2009 ORDER BY budget DESC LIMIT 3;`;
  ...
  it(
    ...
    async done => {
      const query = `SELECT STRFTIME("%Y", release_date) AS year, COUNT(*)
      AS count FROM MOVIES GROUP BY STRFTIME("%Y", release_date) ORDER BY YEAR DESC`;
      const result = await db.selectMultipleRows(query);
  ...
  it(
    ...
    async done => {
      const query = `SELECT STRFTIME("%m", time_created) AS month, COUNT(*) AS count FROM 
      MOVIE_RATINGS GROUP BY STRFTIME("%m", time_created) ORDER BY COUNT DESC;`;
      const result = await db.selectMultipleRows(query);
